      ...
      >
        ...
        <div className="hero-content flex-col lg:flex-row-reverse w-full relative z-10 p-8">
          <div className="flex-1">
            ...
            {item.Type === 'Series' && (
              <p className="badge badge-accent badge-soft rounded-none">TV Series</p> 
            )}            
            {item.ImageTags && item.ImageTags.Logo ? (
              <div className="flex justify-center my-4">
                <img src={item.ImageTags.Logo} className="h-auto w-auto max-h-16 max-w-full object-contain"/>
              </div>
            ) : (
              <h1 className="sm:text-5xl text-3xl font-bold sm:my-6 my-2 text-white drop-shadow-lg">{item.OriginalTitle || item.Name}</h1>
              )
            }
            {item.Overview && (
